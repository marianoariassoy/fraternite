import { Link } from 'wouter'

const Novedades = () => {
  return (
    <section
      className='text-white bg-secondary'
      id='Novedades'
    >
      <div className='m-auto max-w-7xl px-6 py-12'>
        <h1 className='font-extrabold mb-8'>NOVEDADES</h1>
        <section className='grid lg:grid-cols-4 gap-8 items-start'>
          <article className='flex flex-col gap-y-4'>
            <div>
              <div className='w-full py-1 px-4 bg-primary rounded-md mb-2'>Salta</div>
              <p className='text-wrap text-sm font-bold'>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,</p>
            </div>
            <Link href={'/post/1'}>
              <a className='relative'>
                <div className='bg-black/50 backdrop-blur-sm h-full w-full absolute flex justify-center items-center text-white font-bold text-4xl opacity-0 hover:opacity-100 transition-all cursor-pointer'>
                  +
                </div>
                <img
                  src='https://images.pexels.com/photos/6753871/pexels-photo-6753871.jpeg?auto=compress&cs=tinysrgb&w=1600&lazy=load'
                  alt=''
                  className='aspect-video lg:aspect-square object-cover h-full w-full'
                />
              </a>
            </Link>

            <p className='text-wrap text-sm'>
              Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut
              laoreet dolore magna aliquam erat volutpat. Ut wisi enim ad minim veniam, quis nostrud exerci tation
              ullamcorper suscipit lobortis nisl ut aliquip ex ea commodo conse
            </p>
          </article>
          <article className='flex flex-col gap-y-4'>
            <div>
              <div className='w-full py-1 px-2 bg-primary rounded-md mb-2'>Salta</div>
              <p className='text-wrap text-sm font-bold'>Lorem ipsum dolor sit amet, consectetuer adipiscing elit</p>
            </div>
            <div className='relative'>
              <div className='bg-black/50 backdrop-blur-sm h-full w-full absolute flex justify-center items-center text-white font-bold text-4xl opacity-0 hover:opacity-100 transition-all cursor-pointer'>
                +
              </div>
              <img
                src='https://images.pexels.com/photos/6753871/pexels-photo-6753871.jpeg?auto=compress&cs=tinysrgb&w=1600&lazy=load'
                alt=''
                className='aspect-video lg:aspect-square object-cover h-full w-full'
              />
            </div>
            <p className='text-wrap text-sm'>
              Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut
              laoreet dolore magna aliquam erat volutpat. Ut wisi enim ad minim veniam, quis nostrud exerci tation
              ullamcorper suscipit lobortis nisl ut aliquip ex ea commodo conse
            </p>
          </article>
          <article className='flex flex-col gap-y-4'>
            <div>
              <div className='w-full py-1 px-2 bg-primary rounded-md mb-2'>Salta</div>
              <p className='text-wrap text-sm font-bold'>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,</p>
            </div>
            <div className='relative'>
              <div className='bg-black/50 backdrop-blur-sm h-full w-full absolute flex justify-center items-center text-white font-bold text-4xl opacity-0 hover:opacity-100 transition-all cursor-pointer'>
                +
              </div>
              <img
                src='https://images.pexels.com/photos/6753871/pexels-photo-6753871.jpeg?auto=compress&cs=tinysrgb&w=1600&lazy=load'
                alt=''
                className='aspect-video lg:aspect-square object-cover h-full w-full'
              />
            </div>
            <p className='text-wrap text-sm'>
              Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut
              laoreet dolore magna aliquam erat volutpat. Ut wisi enim ad minim veniam, quis nostrud exerci tation
              ullamcorper suscipit lobortis nisl ut aliquip ex ea commodo conse
            </p>
          </article>
          <article className='flex flex-col gap-y-4'>
            <div>
              <div className='w-full py-1 px-2 bg-primary rounded-md mb-2'>Salta</div>
              <p className='text-wrap text-sm font-bold'>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,</p>
            </div>
            <div className='relative'>
              <div className='bg-black/50 backdrop-blur-sm h-full w-full absolute flex justify-center items-center text-white font-bold text-4xl opacity-0 hover:opacity-100 transition-all cursor-pointer'>
                +
              </div>
              <img
                src='https://images.pexels.com/photos/6753871/pexels-photo-6753871.jpeg?auto=compress&cs=tinysrgb&w=1600&lazy=load'
                alt=''
                className='aspect-video lg:aspect-square object-cover h-full w-full'
              />
            </div>
            <p className='text-wrap text-sm'>
              Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut
              laoreet dolore magna aliquam erat volutpat. Ut wisi enim ad minim veniam, quis nostrud exerci tation
              ullamcorper suscipit lobortis nisl ut aliquip ex ea commodo conse
            </p>
          </article>
        </section>
      </div>
    </section>
  )
}

export default Novedades
